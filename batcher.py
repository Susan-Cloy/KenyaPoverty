...
class Batcher():
    def __init__(self, tfrecord_files, label_name, scalar_features, ls_bands='rgb', nl_band=None, nl_label=None, batch_size=1, epochs=1,
                 normalize=None, shuffle=False, augment=False, clipneg=True, cache=False, num_threads=1):
        '''
        Args
        ...
        if normalize is not None and normalize not in _MEANS_DHS:
    ...
    def get_batch(self):
        ...
        if self.shuffle:
            ...
            dataset = (tf.data.Dataset.from_tensor_slices(self.tfrecord_files).shuffle(buffer_size=1000).interleave(
                      lambda file_path:tf.data.TFRecordDataset(file_path, compression_type='GZIP'), cycle_length=self.num_threads,
                      block_length=1, num_parallel_calls=tf.data.experimental.AUTOTUNE))
        else:
            # convert to individual records
            dataset = tf.data.TFRecordDataset(filenames=self.tfrecord_files, compression_type='GZIP', buffer_size=1024 * 1024 * 128,
        ...
        # prefetch 2 batches at a time to smooth out the time taken to load input files as we go through shuffling and processing
        ...
        # batch then repeat => batches respect epoch boundaries i.e. last batch of each epoch might be smaller than batch_size
    ...
    def process_tfrecords(self, example_proto):
        ...
        # dataset_constants for the bands 
        _MEANS_DHS = {'BLUE':  0.059183, 'GREEN': 0.088619, 'RED':   0.104145, 'SWIR1': 0.246874, 'SWIR2': 0.168728, 'TEMP1': 299.078023,
                      'NIR':   0.253074, 'DMSP':  4.005496, 'VIIRS': 1.096089}
        
        _STD_DEVS_DHS = {'BLUE':  0.022926, 'GREEN': 0.031880, 'RED':   0.051458, 'SWIR1': 0.088857, 'SWIR2': 0.083240, 'TEMP1': 4.300303,
                         'NIR':   0.058973, 'DMSP':  23.038301, 'VIIRS': 4.786354}

        img = float('nan')
        if len(ex_bands) > 0:
            if self.normalize is not None:
                means = _MEANS_DHS[self.normalize]
                std_devs = _STD_DEVS_DHS[self.normalize]

            # for each band, reshape to (255, 255) and crop to (224, 224) then subtract mean and divide by std dev
            for band in ex_bands:
    ...
    def split_nl_band(self, ex):
        ...
        ex['images'] = tf.cond(year < 2012,
                               true_fn=lambda: tf.concat([img, all_0], axis=2), # if DMSP, then add an all-0 VIIRS band to the end
                               false_fn=lambda: tf.concat([img[:, :, 0:-1], all_0, img[:, :, -1:]], axis=2) # if VIIRS, then insert an all-0                                                                                                               DMSP band before the last band
                              )
        return ex
